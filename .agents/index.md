...
| `action/` | Action entry + generated `action/dist/` release artifact (gitignored locally) |
...
| `pnpm build` | `tsc` → `dist/` |
| `pnpm build:action` | `ncc build action/index.ts -o action/dist` (regenerates the ignored release bundle) |
...
- **Change Action inputs/outputs**: `action/index.ts` + root `action.yml` + `README.md`, then run `pnpm build:action`; the release workflow force-adds `action/dist/`.

## Generated artifacts

- `dist/` — `pnpm build` (tsc) output; gitignored.
- `action/dist/` — `pnpm build:action` (ncc) output; **gitignored locally and force-added by `release.yml`** for the published release. Never hand-edit. Root `action.yml` points at `action/dist/index.js`.
